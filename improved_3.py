Yes, here is the complete, integrated script with the progress bar and model-saving methods added.

The key changes are the import of `tqdm` and the additions within the `if __name__ == "__main__"` block to wrap the data loaders and save the final model.
...
from tqdm import tqdm # MODIFIED: Import tqdm
...
class Config:
    ...
    # ADDED: Path to save the fine-tuned model
    output_model_path: str = "./finetuned_sentiment_model"
    # Data and Splitting
...
if __name__ == "__main__":
    ...
    for epoch in range(config.epochs):
        ...
        # MODIFIED: Training with tqdm progress bar
        for x_batch, y_batch in tqdm(train_dataset, desc="Training"):
            train_step(x_batch, y_batch, student_model, optimizer, loss_functions, metrics_map, config.temperature, config.alpha)

        # MODIFIED: Validation with tqdm progress bar
        for x_batch_val, y_batch_val in tqdm(val_dataset, desc="Validating"):
            eval_step(x_batch_val, y_batch_val, student_model, loss_functions[1], metrics_map)

        print(f"Epoch Summary | Train Loss: {metrics_map['train_loss'].result():.4f} | "
              f"Val Loss: {metrics_map['val_loss'].result():.4f} | "
              f"Val Accuracy: {metrics_map['val_accuracy'].result():.4f}")

    # ADDED: Save the fine-tuned model and tokenizer
    print(f"\n--- Saving fine-tuned model to {config.output_model_path} ---")
    student_model.save_pretrained(config.output_model_path)
    student_tokenizer.save_pretrained(config.output_model_path)
    print("Model and tokenizer saved successfully.")

    # Final evaluation on the test set
    print("\n--- Final Evaluation on Test Set ---")
    test_accuracy_metric = metrics.SparseCategoricalAccuracy(name='test_accuracy')
    # MODIFIED: Testing with tqdm progress bar
    for x_batch_test, y_batch_test in tqdm(test_dataset, desc="Testing"):
